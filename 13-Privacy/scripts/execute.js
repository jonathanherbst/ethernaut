...
CONTRACT_NAME = "Privacy"

async function getContract(wallet) {
  const factory = await hre.ethers.getContractFactory(CONTRACT_NAME);
  let contract = null;
  if(hre.network.name == "hardhat") {
    const randNum = [ethers.utils.randomBytes(32), ethers.utils.randomBytes(32), ethers.utils.randomBytes(32)];
...
async function main() {
  ...
  const slot6 = (await ethers.provider.getStorageAt(contract.address, 5));
  const passwd = ethers.utils.arrayify(slot6).slice(0, 16);
